...
public class Parser {
    ...
    public String query(String queryStatment) {
        ...
        else if (words[0].equalsIgnoreCase("update")&&validateUpdateQuery(queryStatment)) {
            queryResponse = nextStage.parseUpdate(queryStatment);
    ...
    private static boolean validateStartOfQuery(String query) {
        ...
        // query
        return trimmedQuery.toUpperCase().matches("(?i)\\s*(CREATE TABLE|SELECT|INSERT INTO|DELETE FROM|UPDATE)\\s+.+");
    }
...
public static boolean validateInsertQuery(String query) {
    ...
    public static boolean validateUpdateQuery(String query) {
        ...
        if (!query.matches("(?i)\\s*UPDATE\\s+\\w+\\s+SET\\s+.+;\\s*")) {
            System.out.println("Invalid UPDATE syntax. Should be: UPDATE [table] SET [column]=[value] WHERE [condition];");
            return false;
        }

        // Extracting parts between UPDATE, SET, and WHERE
        String[] updateParts = query.split("\\s+SET\\s+", 2);
        if (updateParts.length < 2) {
            System.out.println("Missing 'SET' clause.");
            return false;
        }

        String[] whereParts = updateParts[1].split("\\s+WHERE\\s+", 2);
        String setClause = whereParts[0].trim();
        String whereClause = (whereParts.length > 1) ? whereParts[1] : null;

        // Validate SET clause
        if (!setClause.matches("(?i)\\w+\\s*=\\s*('[^']*'|\\d+)(\\s*,\\s*\\w+\\s*=\\s*('[^']*'|\\d+))*")) {
            System.out.println("Invalid syntax in 'SET' clause.");
            return false;
        }

        // Validate WHERE clause if it exists
        if (whereClause != null && !whereClause.trim().endsWith(";")) {
            System.out.println("WHERE clause must end with a semicolon ';'.");
            return false;
        }

        if (whereClause != null && !whereClause.matches("(?i)\\w+\\s*(=|>|<|<>|!=)\\s*('[^']*'|\\d+)(\\s+(AND|OR)\\s+\\w+\\s*(=|>|<|<>|!=)\\s*('[^']*'|\\d+))*;")) {
            System.out.println("Invalid syntax in 'WHERE' clause.");
            return false;
        }
